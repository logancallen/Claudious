...
### 2026-04-23 — GOTCHA — PowerShell script exit code 255 via `| Select-Object -First N` broken pipe

**Severity:** LOW
**Context:** `scripts/update-heartbeat.ps1` exited 0 in-process (`.\script.ps1`) but 255/-1 when invoked as `& powershell.exe -File script.ps1 | Select-Object -First 8`. Same script, same payload, no PowerShell parser/runtime errors, heartbeat file written correctly both times. Session #2 Part C initially flagged this as a real script failure; Session #3 native retest isolated it to the pipeline-truncation pattern: `Select-Object -First N` closes the pipeline before `Get-Content $heartbeatFile` finishes streaming, so the upstream process sees a broken pipe and the wrapper reports non-zero even though the script itself returned 0.
**Learning:** Never truncate a PowerShell pipeline mid-stream when you also care about the upstream exit code. If you need the first N lines, `Out-String | ForEach-Object { ($_ -split "`n")[0..7] }` or write to a temp file and read that, rather than `| Select-Object -First N`. For CC hooks or scheduled tasks that care about exit code, invoke the script without a truncating downstream pipe — and always add `[Console]::Out.Flush(); exit 0` at the end so buffering doesn't interfere.
**Applies to:** Any PowerShell script consumed by a process that cares about `$LASTEXITCODE` or shell exit propagation (CC hooks, CI, scheduled tasks). Confirmed 2026-04-23 by running the preflight-mode path (no truncating downstream pipe) — native exit 0. Also informs Session #3 preflight hook design: wrapper calls `& $script -Preflight $currentRepo` with no pipeline consumer.
